...
export default function VotoPanel({
  ...
  estado, onEstado,
  e1, onE1, e2, onE2,
  restaurantes, puedeFlexible, onEnviar, sending, disabled,
}) {
  const nombres = restaurantes.map((r) => r.nombre);
  const flexible = e1 === FLEX;
        ...
        <>
          ...
          <Field id="comidas-e1" label="Prioridad 1">
            <select id="comidas-e1" className={SELECT_CLS} value={e1} onChange={(e) => onE1(e.target.value)}>
              ...
              {/* Hasta que alguien elija restaurante no hay nada a lo que
                  unirse: el primero en votar tiene que mojarse. */}
              <option value={FLEX} disabled={!puedeFlexible}>
                {`★ ${FLEX}`}{puedeFlexible ? "" : " — aún no ha votado nadie"}
              </option>
            </select>
          </Field>

          {/* La prioridad 2 es opcional y suma al ranking igual que la 1. No
              se pide a quien va flexible: no expresa preferencia. */}
          {!flexible && (
